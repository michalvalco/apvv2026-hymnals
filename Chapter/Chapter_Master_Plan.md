...
**v0.4 (2026-06-03 — Halama Ch. 5 documented; §5 caveat created and audit-corrected; Hrejsa elevated; path/ID housekeeping):**
- Event: `Sources/JHalama_2003.md` documented (PARTIAL — Ch. 5 intro pp. 154–158 + concluding "Celková charakteristika" pp. 189–192 only; chapter body and rest of monograph not read). `Sources/Source_Index.md` row bumped 📋 → 📝 (partial) with scope note. Disambiguation (Jindřich ≠ Ota Halama) preserved.
- Event: `Chapter/S5_Chronology_Caveat.md` created, then **audit-corrected the same day** against `GresovaSek_Prispevok` and `taxonomy/hymn_pairs.csv`. Three first-draft errors fixed: (i) the Utraquist **Kolínský 1517** had been mis-filed as a Brethren source — it is CS's largest *melodic* source (p. 6) but Utraquist, so it belongs under the David-corpus framing, not this caveat; (ii) melodic vs textual provenance now distinguished — CS's largest single *textual* source is the **Lutheran Závorkov 1602** (p. 7), so the Brethren textual contribution (Roh 1541, 3rd) is a minority strand; (iii) the Eucharistic application was overstated — PAIR_002 is `CONJECTURED`/`UNVERIFIED`, and Grešová-Sekelská (2021, pp. 8–9) suggests Tranovský *excluded* Brethren Eucharistic content, so the caveat currently has no verified Brethren→CS pair to stand on.
- Decision: §5 chronology caveat adopted as a **framing principle**. The genuinely Brethren sources feeding CS — Kancionál Jana Roha (`KancRoh_1541`, 1541), Šamotulský 1561, Ivančický 1564 — predate the Brethren Calvinisation Halama dates to c. 1600–1607 (`JHalama_2003` pp. 154–155, 190). The transfer boundary is therefore Lutheran ↔ older-Lukán Brethren, relocatable onto Luke's modes-of-being grammar (`Crews_2018` pp. 22–23). Illustration awaits the PAIR_002 resolution (Vajda 1944 pp. 74–84 + Mladějovská 2008 + CS-vs-Ivančický collation).
- Decision: **Hrejsa 1912 elevated** to a §4.1 + §2 co-cornerstone in the Tier-2 acquisitions queue (alongside Just/Rothkegel 2012). It already gates §4.1 drafting (CB-1575 refutation) and is now independently required for the §2 doctrinal-dispute narrative on which Halama leans (Hrejsa cited at `JHalama_2003` pp. 377, 393ff, 401: Mladá Boleslav 1598 / Žeravice 1604 disputes + CB reception). Acquisition route unchanged: Comenius print or ILL via ETF UK Praha.
- Flag (chapter framing): `GresovaSek_Prispevok` pp. 6–7 shows CS's adopted material is predominantly **Lutheran/German** (Závorkov 1602 + German translations the largest single group); the Brethren textual contribution is real but minority. The thesis's "Brethren kancionál → CS" emphasis must be calibrated to scale in §5. Also p. 9 (via Mocko): two "Calvinist errors" slipped Tranovský's Lutheran filter into CS — a §4.1/§5 footnote candidate.
- Housekeeping — short-ID inconsistency (Michal's call): the 1912 *Česká konfesse* carries **two** short IDs — `Hrejsa_CeskaKonf` (Source_Index L97) and `Hrejsa_1912` (this plan + the two new notes). Distinct from the genuine `Hrejsa_LutKalv_1934` (the 1934 ČČH study). **RESOLVED 2026-06-03 (Michal's decision):** standardised on `Hrejsa_1912`; the former `Hrejsa_CeskaKonf` retired and propagated across `Source_Index.md` (defining entry + both cross-references) and `Sources/Hruby_1939.md`. The 1934 study is untouched.
- Housekeeping — paths corrected: Windows user is `Michal Valco`; repo at `C:\Users\Michal Valco\Documents\OneDrive\Documents\GitHub\apvv2026-hymnals\` (note the nested `Documents\OneDrive\Documents`). `CLAUDE.md` grant-admin path prefix fixed and the canonical repo path recorded there. The earlier on-record prefix `C:\Users\valco\OneDrive\Documents\...` was wrong on both username and nesting.
